...
type Props = {
  ...
  neighborDemandCount: number;
};

function HeroNeighborHeading({ count }: { count: number }) {
  if (count === 1) {
    return (
      <>
        <StylizedUnderline underlineClassName="text-sun">Un voisin</StylizedUnderline>
  ...
  if (count > 1) {
    return (
      <>
        <StylizedUnderline underlineClassName="text-sun">
          {count} voisins
        </StylizedUnderline>
        {" "}ont besoin de vous aujourd&apos;hui 💛
      </>
    );
  }
...
export function AccueilHero({ userFirstName, neighborDemandCount }: Props) {
      ...
      <div className="relative z-10 flex min-h-[200px] flex-col justify-between p-5 md:min-h-[220px] lg:p-8">
        <div className="max-w-[min(70%,11rem)] space-y-2 md:max-w-[min(100%,32rem)] md:space-y-3 lg:max-w-[min(55%,36rem)]">
          ...
          <h2 className="text-base font-bold leading-6 text-white md:text-[28px] md:leading-9">
            <HeroNeighborHeading count={neighborDemandCount} />
          </h2>
        </div>
        {hasNeighborDemands ? (
          <Link
            href={`${ROUTES.annonces.list}${buildAnnouncementListQuery({ date: "today", tri: "recent" })}`}
            className={cn(heroCtaClass, "shrink-0")}
          >
...
function QuickActionCard({ label, gradient, icon, onClick }: QuickActionProps) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="flex w-full flex-col items-center justify-center gap-1.5 rounded-xl border border-border/60 bg-surface p-3 shadow-card transition hover:scale-[1.02] hover:border-purple/40 md:flex-row md:gap-3 md:p-4"
    >
      <span
        className={cn(
...
export function AccueilQuickActions() {
  ...
  const demandeConfig = getAnnouncementTypeConfig("demande")!;
  const offreConfig = getAnnouncementTypeConfig("offre")!;
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-3 gap-2 md:gap-3">
        <QuickActionCard
          label="Je demande"
          gradient={demandeConfig.gradient}
          icon={
